...
if(~process.argv.indexOf("--elixir-mode")) process.elixirMode = true;
...
function prepareResult(res, isOneLiner, isElixir) {
    ...
    try {
      console.log("res: " +res)
        ...
        if (isElixir) {
            if (isOneLiner) {
                cake = cake.map(function(a){ return /\D\S*/.exec(a)[0]})
                cake = cake.join("").split(".");
                last = cake[cake.length - 1] || cake[cake.length - 2]
                console.log(last);
